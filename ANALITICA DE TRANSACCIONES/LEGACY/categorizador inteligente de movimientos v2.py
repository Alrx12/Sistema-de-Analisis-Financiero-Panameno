...
class FinancialClassifier:
    ...
    def _contains_user_name(self, detail_upper: str) -> bool:
        ...
        # Match de al menos 2 tokens del nombre (para "ALEXIS PINEDA" → 2 hits)
    ...
    def _is_transfer_type(self, row) -> bool:
        ...
        transfer_keywords = ['ACH', 'XPRESS', 'TRANSFERENCIA', 'TRANSFER', 'TRF']
    ...
    def predict(self, row):
        """
        Orden de prioridad:
          0. Detección de nombre propio  ← NUEVO (mayor prioridad)
          1. Match exacto en knowledge base
          2. Patrones regex aprendidos
          3. Fallback por tipo de movimiento
        """
        detail = str(row.get('Detalle', '')).strip().upper()

        # ── PASO 0: Detección de nombre de usuario ───────────────────────────
        # Si el nombre del usuario aparece en el detalle de una transferencia,
        # es con certeza una transferencia entre sus propias cuentas.
        if self.user_name and self._contains_user_name(detail):
            return self.OWN_TRANSFER_CATEGORIES, 1.0, "own_transfer:name_match"

        # Si es una transferencia pero NO contiene el nombre del usuario,
        # se clasifica como transferencia a tercero con alta confianza.
        if self._is_transfer_type(row) and not self._contains_user_name(detail):
            # Solo aplica este paso automático si hay nombre de usuario configurado,
            # de lo contrario dejamos que el resto del pipeline decida.
            if self.user_name:
        ...
        # ── PASO 3: Fallback por tipo de movimiento ──────────────────────────
        tipo = str(row.get('Tipos de Movimientos', '')).upper()
...
if __name__ == "__main__":
    ...
    # En producción: obtener el nombre del usuario desde la sesión/base de datos.
    # Por ahora se pasa como argumento o usa el valor por defecto.
    # Ejemplo de uso en app: main_workflow(user_name=session.user.full_name)
    USER_NAME = "Alexis Pineda"  # ← Cambiar por el nombre del usuario autenticado
