"""
SISTEMA DE PROCESAMIENTO DE DATOS BANCARIOS - PANAMÁ v2.0
...
class Transaccion:
    ...
    banco_origen: str
    metodo_pago: str
    archivo_origen: str = ""  # Nuevo: tracking de archivo fuente
...
class ConfiguracionUsuario:
    ...
    def __post_init__(self):
        # Si no se especifica carpeta, usar una por defecto en el directorio de ejecución
...
class FinancialDataProcessor:
    ...
    def procesar_archivo(self, ruta_archivo: str, banco_manual: Optional[str] = None) -> List[Transaccion]:
        ...
        # Verificar que el archivo esté en la carpeta del usuario (seguridad básica)
        archivo_path = Path(ruta_archivo).resolve()
        user_path = Path(self.usuario_actual.carpeta_datos).resolve()
...
def ejemplo_flujo_completo():
    ...
    print("🏦 SISTEMA DE ANÁLISIS FINANCIERO MULTI-USUARIO")
    print("=" * 60)
    
    user_manager = UserManager()
    
    # 2. Crear usuario (o usar existente)
    # En tu caso, manejas 3 bancos
    ...
    # 4. Opción A: Procesar archivos específicos (tu caso actual)
    print("\n📂 OPCIÓN A: Procesar archivos específicos")
    print("-" * 60)
    
    # En tu PC, copia los archivos a la carpeta del usuario o usa rutas absolutas
    # La carpeta se crea automáticamente en: ./datos_bancarios/alexis_pineda/
    
    # Ejemplo con rutas absolutas (tu estructura actual):
    archivos_manuales = [
    ...
    # 5. Opción B: Procesar toda la carpeta del usuario (para app móvil)
    # processor.limpiar_transacciones()
...
def ejemplo_usuario_un_banco():
    """Ejemplo para un usuario que solo usa un banco"""
    
    user_manager = UserManager()
    
    # Usuario con solo BAC
    usuario = user_manager.crear_usuario(
        user_id="usuario_simple",
        nombre="Usuario BAC",
        bancos=["BAC Credomatic"]
    )
    
    processor = FinancialDataProcessor(user_manager)
    processor.inicializar_para_usuario("usuario_simple")
    
    # Solo procesará archivos de BAC, ignorará otros
    # processor.procesar_carpeta_usuario()
    
    print(f"\n✓ Usuario configurado con: {usuario.bancos_activos}")

if __name__ == "__main__":
    # Ejecutar ejemplo principal
    processor, analyzer = ejemplo_flujo_completo()
    
    # Descomenta para ver ejemplo de usuario simple:
    # ejemplo_usuario_un_banco()