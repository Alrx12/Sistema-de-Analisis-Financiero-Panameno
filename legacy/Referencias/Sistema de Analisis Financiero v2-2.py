"""
SISTEMA DE PROCESAMIENTO DE DATOS BANCARIOS - PANAMÁ v2.2
    ...
    def cargar_archivo(self, ruta_archivo: str) -> pd.DataFrame:
        ...
        try:
            ...
            if extension == '.csv':
                ...
                # CORRECCIÓN: Manejo mejorado de xlrd
                if extension == '.xls':
                    try:
                        import xlrd
...
# ============================================================
# SECCIÓN 6: MOTOR DE ANÁLISIS E INSIGHTS
# ============================================================
...
# ============================================================
# SECCIÓN 7: INTERFAZ DE USUARIO INTERACTIVA
# ============================================================

def mostrar_menu_principal():
    """Muestra el menú principal del sistema"""
    print("\n" + "="*60)
    print("🏦 SISTEMA DE ANÁLISIS FINANCIERO MULTI-USUARIO v2.2")
    print("="*60)
    print("1. Iniciar sesión (usuario existente)")
...
def seleccionar_bancos() -> List[str]:
    """Permite al usuario seleccionar qué bancos va a usar"""
    bancos_disponibles = ['Banco General', 'Banistmo', 'BAC Credomatic']
    bancos_seleccionados = []
    
    print("\n🏦 Selecciona los bancos que vas a utilizar:")
    print("(Ingresa los números separados por comas, ej: 1,2)")
    ...
    try:
        ...
        print("⚠ Selección inválida. Usando todos los bancos por defecto.")
...
def crear_nuevo_usuario(user_manager: UserManager) -> Optional[ConfiguracionUsuario]:
    ...
    user_id = input("ID de usuario (sin espacios, ej: alexis_pineda): ").strip().lower()
    if not user_id:
        print("⚠ ID inválido")
        return None
    
    if user_id in user_manager.usuarios:
        print(f"⚠ El usuario '{user_id}' ya existe. Usa 'Iniciar sesión'.")
        return None
...
def iniciar_sesion(user_manager: UserManager) -> Optional[ConfiguracionUsuario]:
    ...
    for i, (uid, user) in enumerate(user_manager.usuarios.items(), 1):
        print(f"  {i}. {user.nombre} ({uid}) - Bancos: {', '.join(user.bancos_activos)}")
    
    user_id = input("\nIngresa tu ID de usuario: ").strip().lower()
...
def procesar_archivos_usuario(processor: FinancialDataProcessor, usuario: ConfiguracionUsuario):
    """Flujo para procesar archivos del usuario"""
    print("\n" + "="*60)
    print("📂 PROCESAR ARCHIVOS BANCARIOS")
    print("="*60)
    print("Opciones:")
    print("1. Procesar archivos desde mi carpeta de datos")
    print("2. Procesar archivos desde rutas específicas")
    ...
    if opcion == "1":
        ...
        print("\nIngresa las rutas de los archivos (una por línea, línea vacía para terminar):")
        ...
        if opcion == "1":  # Iniciar sesión
            ...
            if usuario:
                ...
                if processor.inicializar_para_usuario(usuario.user_id):
                    ...
                    while True:
                        print("\n" + "="*60)
                        print(f"👤 {usuario.nombre} | Bancos: {', '.join(usuario.bancos_activos)}")
                        print("="*60)
                        ...
                        if sub_opcion == "1":
                            ...
                            if processor.transacciones:
                                ...
                                print("⚠ No hay transacciones procesadas. Procesa archivos primero.")
                        
                        elif sub_opcion == "3":
                            if processor.transacciones:
                                analyzer = FinancialAnalyzer(processor.transacciones, usuario)
                                ruta = os.path.join(usuario.carpeta_datos, "reporte_financiero.json")
            ...
            nuevo_usuario = crear_nuevo_usuario(user_manager)
            if nuevo_usuario:
                print(f"\n✓ Usuario '{nuevo_usuario.nombre}' creado exitosamente.")
                print(f"  Ahora puedes iniciar sesión con ID: {nuevo_usuario.user_id}")
            ...
            if usuarios:
                ...
                for u in usuarios:
                    ...
                    print(f"    Bancos: {', '.join(u.bancos_activos)}")
            else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Instala con: pip install pandas numpy openpyxl xlrd")
        sys.exit(1)
