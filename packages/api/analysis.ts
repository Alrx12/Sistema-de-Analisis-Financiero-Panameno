...
export async function listAnalysis(): Promise<AnalysisSnapshot[]> {
  const res = await getApiClient().get<AnalysisSnapshot[]>("/analysis/")
  return res.data
}

export async function getAnalysis(id: string): Promise<AnalysisSnapshot> {
  const res = await getApiClient().get<AnalysisSnapshot>(`/analysis/${id}`)
  return res.data
}
...
export async function getTransactions(
  ...
  const res = await getApiClient().get<Transaction[]>(
    `/analysis/${snapshotId}/transactions`,
    { params }
  )
  return res.data
}
...
export async function getConfidenceStats(
  ...
  const res = await getApiClient().get<ConfidenceStats>(
    `/analysis/${snapshotId}/confidence-stats`
  )
  return res.data
}
...
export async function getAggregatedSummary(params: {
  ...
  const res = await getApiClient().get<AggregatedSummary>("/analysis/aggregated", {
    params,
  })
  return res.data
}
...
export async function reclassifyTransaction(
  ...
  const res = await getApiClient().post<Transaction>(
